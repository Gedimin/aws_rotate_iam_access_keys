...
def _get_sts_token_response():
    ...
    role_arn = '' # put here ARN of AWS IAM Role which will be assumed
    aws_profile = 'sso-user' # put here your value of aws profile to get token
    session_name = 'AWSCLI-Session'
    command = f'aws sts assume-role --role-arn {role_arn} --role-session-name {session_name} --profile {aws_profile}'
...
if __name__ == '__main__':
    ...
    if os.path.exists(aws_credentials_file):
        ...
        print(f'File {aws_credentials_file} does not exist. Please make initian configuration of aws-cli')
